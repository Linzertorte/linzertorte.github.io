...
export default function Study() {
  ...
  const [card, setCard] = useState(null); const [done, setDone] = useState(false);
  const [error, setError] = useState(''); const [submitting, setSubmitting] = useState(false);
  const loadNext = useCallback(async id => { try { const data = await api(`/api/books/${id}/next?today=${todayNumber()}`); setDone(Boolean(data.done)); setCard(data.card || null); } catch (error) { setError(error.message); } }, []);

  useEffect(() => {
    if (!bookId) { navigate('/', { replace: true }); return; }
    api('/api/books').then(({ books }) => {
      if (!books.some(book => book.book_id === bookId)) throw new Error('That deck is not available.');
      return loadNext(bookId);
    }).catch(error => setError(error.message));
  }, [bookId, loadNext, navigate]);

  async function review(rating) {
    if (!card || !bookId) return;
    setSubmitting(true);
    try { await api(`/api/books/${bookId}/review`, { method: 'POST', body: JSON.stringify({ word_id: card.word_id, rating, today: todayNumber() }) }); await loadNext(bookId); }
